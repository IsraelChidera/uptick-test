...
const Jobs = () => {
  ...
  if (error) {
    return (
      <div className="w-[300px] h-[calc(100vh-200px)] flex justify-center items-center mx-auto">
        <span className="text-red-400">Error: {error.message}</span>
      </div>
    );
  }

  if (data?.data && !isLoading && data?.data?.length <= 0) {
    return (
      <div className="w-[300px] h-[calc(100vh-200px)] flex justify-center items-center mx-auto">
        <EmptyStateUI message={'No Job to Display'} />
      </div>
    );
  }
